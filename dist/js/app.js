(() => {
    ...
    function voting() {
        document.addEventListener("DOMContentLoaded", (function() {
            setTimeout((() => {
                let slider = document.querySelector(".swiper").swiper;
                if (!slider) return;
                initVoting(slider);
            }), 300);
        }));
        function initVoting(slider) {
            let voteButton = document.querySelector(".voting__hurt");
            let likeIcon = voteButton.querySelector(".voting__like");
            let likeActiveIcon = voteButton.querySelector(".voting__like-active");
            function updateLikeButton() {
                let currentIndex = slider.realIndex;
                let photoId = currentIndex + 1;
                if (localStorage.getItem("voted_" + photoId) || document.cookie.includes("voted_" + photoId)) {
                    likeIcon.style.display = "none";
                    likeActiveIcon.style.display = "block";
                } else {
                    likeIcon.style.display = "block";
                    likeActiveIcon.style.display = "none";
                }
            }
            slider.on("slideChange", updateLikeButton);
            updateLikeButton();
            voteButton.addEventListener("click", (function() {
                ...
                fetch("vote.php", {
                    ...
                    if (data.success) {
                        document.cookie = `voted_${photoId}=1; max-age=2592000; path=/`;
                        localStorage.setItem("voted_" + photoId, "1");
                        likeIcon.style.display = "none";
                        likeActiveIcon.style.display = "block";
